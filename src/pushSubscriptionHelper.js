...
export async function subscribeUserToPush(apiBaseUrl, token) {
  ...
  try {
    ...
    console.log('✅ Push Subscription created (TL):', subscription);
    ...
    if (response.ok) {
      console.log('✅ Push subscription saved to backend successfully (TL).');
    } else {
      console.error('❌ Failed to save push subscription to backend (TL):', response.statusText);
    }
  } catch (error) {
    console.error('❌ Error during push subscription registration (TL):', error);
  }
}
